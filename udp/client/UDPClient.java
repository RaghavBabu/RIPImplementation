...
public class UDPClient {
	...
	public static void main(String args[]) throws Exception
	{
		...
		Queue<TCPPacket> sendListQueue = new ArrayBlockingQueue<TCPPacket>(10000);
		...
		for(TCPSegment segment : segments){
			...
			seqNoTCPPacketMap.put(seqNo, packet);
			seqNo++;
		}

		//start send and receive Threads
		udpClient.startThreads(client,clientAck, ipAddress, window, sendListQueue);
	}

	private void startThreads(DatagramSocket client, DatagramSocket clientAck, InetAddress ipAddress, CongestionWindow window, Queue<TCPPacket> sendListQueue) {

		SlidingWindowControlThread controlThread = new SlidingWindowControlThread(window, sendListQueue);
		controlThread.start();
		
		SendThread sendThread = new SendThread(client, window, ipAddress);
		sendThread.start();
